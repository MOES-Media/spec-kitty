...
- **Runs / threshold**: `k: 5` minimum on every judge rule (FR-006; the real
  ...
  **`passThreshold` is never a single blanket value across all rows** (an
  ...
  explicit and equal to `ceil(k / 2)` — `3` at `k: 5`. Never rely on the
  implicit runtime default either way: `runner.ts:305,566` resolves an
  omitted `passThreshold` to `Math.ceil(entry.k / 2)` regardless of
  `aggregation`, which silently downgrades an intended pass-k row's "all
  runs must pass" guarantee to a majority vote if the field is left out.
