...
- **Runs / threshold**: `k: 5` minimum on every judge rule (FR-006; the real
  ...
  `passThreshold: ceil(k / 2)` — `3` at `k: 5` — explicit in every manifest,
  never the implicit default. Avoidance-boundary (safety-adjacent) rules use
  `aggregation: pass-k` (all 5 runs must pass).
