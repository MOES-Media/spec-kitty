...
| FR-006 | `k ≥ 5`; pass^k rows (`AVOIDANCE-BOUNDARY-*`, **and `CAPABILITY-CONTAINMENT-*` — Finding 2a**) set `aggregation: pass-k` **and `passThreshold` equal to `k`** (Finding 2b); k-of-n rows set `passThreshold: ceil(k/2)` | `yq -e '[.rules[] \| select(.ruleId \| test("^(AVOIDANCE-BOUNDARY\|CAPABILITY-CONTAINMENT)")) \| (.passThreshold // .k) == .k] \| all' conformance/behavioral/profiles/architect-alphonso.yaml` | `true`, exit `0` | Fixture with `aggregation: pass-k, k: 5, passThreshold: 3` → manifest load throws (`manifest.ts:299-306`'s own validator); implementers must still write `passThreshold: <k>` explicitly in every committed manifest — never rely on omission, because `runner.ts:305`'s *runtime* default is `ceil(k/2)`, not `k`, and this `yq` check only audits the committed YAML, not the runtime path |
...
| **New — Integration Contract excerpt (Finding 3)** | Every `JudgeAssertion`'s `promptTemplate` embeds the verbatim profile-field excerpt its axis grades | `yq '.rules[] \| select(.ruleId \| test("^AVOIDANCE-BOUNDARY")) \| .promptTemplate' <manifest> \| command grep -qF "$(yq '.specialization."avoidance-boundary"' <source.agent.yaml>)"` (axis-specific field per the Integration Contract table) | match found, exit `0` | `promptTemplate` that only says "consult the profile's avoidance-boundary field" without the literal text → no match, exit `1` |
...
spec.md's FR/C text. None below changes an FR's stated user-observable
behavior — all are verification-design, path, or integration-fidelity
corrections this plan is responsible for supplying. If a future reviewer
judges otherwise for any item, route it back through `spk-mission-specify`
before `/spec-kitty.tasks` runs.*
