...
| `docs/rubric/sop-rule-taxonomy.md` | ships at v1.0.0 only; **no v1.1 appendix** | The **v1.1 appendix** (directive-mapping guidance: which directive fields become `ruleText`, decidability mapping of the 26 directives onto the existing 5 binary + 2 judge classes, citation format for `source.supporting` = directive@SHA) was planned for M2 but is not delivered — it would have required bumping the front-matter version past 1.0.0, breaking the `RUBRIC_VERSION = "1.0.0"` invariant already pinned (with hard-coded test assertions) by muster's merged openclaw-sop adapter (`src/adapters/openclaw-sop/runner.ts:63`). The base document's 5 binary + 2 judge classes are already normative at v1.0.0 — M3 (MOES-Media/spec-kitty#23) checks cite the **existing** classes directly; no appendix is needed for M3. | M3/M4 directive rules (classes) |
